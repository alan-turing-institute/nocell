...
@deftogether[(@defstruct*[spreadsheet ([sheets (listof sheet?)]) #:transparent]
    ...
    A cell contains an expression @racket{cell-xpr} and a list of
    attributes (that determine a cells style).}
...
@deftogether[(@defproc[(expression? [v any/c]) boolean?]
              ...
              @defstruct*[application ([fn builtin?] [args (listof expression?)])
                          #:transparent]
              @defproc[(builtin? [v symbol?]) boolean?])]{
...
@deftogether[(@defstruct*[reference () #:transparent]
              ...
              @defstruct*[(absolute-location location) ([label string?])
              #:transparent]
              @defstruct*[(relative-location location) ([source string?] [target
              string?])
              #:transparent])]{

              References refer to other cells.}

A @deftech{value} is either an @tech{atomic value}, a @tech{matrix}, or a
@tech{reference}.
