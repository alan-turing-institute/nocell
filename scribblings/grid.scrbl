...
@tech{reference} is a pointer to another cell or to a range of cells. Both array
values and references can be generated by, and consumed by, certain built-in
functions, as well as having literal forms.

@margin-note{@bold{Excel}: In Excel, one can write @tech{matrix values} and
