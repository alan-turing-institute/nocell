...
(define (project-code? t)
  (regexp-match? #px"^[[:alpha:]]-[[:alnum:]]+-[[:alnum:]]+$" t))
...
(struct/contract monthly-allocation allocation
  ([month-fraction (hash/c gregor:date? real? #:flat? #t)])
  #:transparent)

(struct/contract journal-allocation allocation
  ([debit-fraction real?]
   [credit-fraction real?])
  #:transparent)
...
(define example-allocations
  (list
   (monthly-allocation "Data science for science and the humanities"
                       "Interesting Ocean"
                       "R-XYZ-001"
                       "Oliver Strickson"
                       (hash (gregor:date 2020 04) 0.5
                             (gregor:date 2020 05) 0.5))
   (monthly-allocation "Data-centric engineering"
                       "Ubiquitous Mist"
                       "R-ABC-999"
                       "Callum Mole"
                       (hash (gregor:date 2020 04) 0.5))
...
;; monthly-allocations->sheet : (listof monthly-allocation?) (listof gregor:date?) -> spreadsheet?
(define (monthly-allocations->sheet allocations dates)
  (spreadsheet
   (list
    (sheet
     (list*
      ...
      (for/list ([a (sort allocations allocation<=?)]
                 ...
                 (or #;(member (allocation-client a) '("Corporate Duties"
                                                     ...
                                                     "UNAVAILABLE"))
                     (andmap zero? (hash-values (monthly-allocation-month-fraction a)))))
        ...
        (list* (cell (allocation-client a) '())
               ...
               (map (λ (mon)
                      (cell (hash-ref (monthly-allocation-month-fraction a) mon 0.0)
                            '(output)))
                    dates))))))))


(define (journal-allocations->sheet allocations dates)
  (spreadsheet
   (list
    (sheet
     (list*
      ;; first row: column headings
      (list* (cell "Description" '(column-label))
             (cell "Cost centre" '(column-label))
             (cell "Project" '(column-label))
             (cell "Person" '(column-label))
             (cell "FTE Debit" '(column-label))
             (cell "FTE Credit" '(column-label)))
      
      ;; other rows: allocation entries, with some exclusions
      (for/fold ([result '()])
                ([a (sort allocations allocation<=?)]
                 #:unless
                 (and (= 0 (journal-allocation-debit-fraction a))
                      (= 0 (journal-allocation-credit-fraction a))))

        (append result
                (list
                 (list* (cell "Salary recharges" '()) ; description
                        (cell "RES-2130" '()) ; cost centre
                        (cell (allocation-code a) '())
                        (cell (allocation-person a) '())
                        (cell (journal-allocation-debit-fraction a) '())
                        (cell "0" '()))
...
(define (make-report-ods allocations dates)
  (sxml->ods
   (sheet-spreadsheet->sxml (monthly-allocations->sheet allocations dates)
                       #:blank-rows-before '(1)
                       #:blank-cols-before '(1))
   #:type 'ods))
